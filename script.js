...
function populateRadioButtons() {
   ...
   options.forEach((option, index) => {
      // Create a wrapper for the radio and label
      ...
      if (option === "WiFi") radioButton.checked = true; // Default to WiFi
      ...
      // Append radio button and label to wrapper
...
function addToCart(name, category, subCategory, price, tcp) {
   ...
   <td data-field="protocol" style="font-weight: 600; color: var(--secondary)">${protocol}</td>
   <td class="editable" data-field="socketQuantity">${socketQuantity}</td>
   <td class="editable" data-field="rj45Quantity">${rj45Quantity}</td>
   <td style="color: var(--text-muted)">Rs ${itemCP.toFixed(2)}</td>
   <td style="font-weight: 700; color: var(--primary-dark)">Rs ${itemSP.toFixed(2)}</td>
   <td><input type="text" class="notes-input" placeholder="Notes..." style="padding: 4px; font-size: 12px;"></td>
   <td>
      <div style="display: flex; gap: 0.5rem;">
        <button class="edit-btn" onclick="editRow(this)" style="padding: 6px 10px; font-size: 12px; background-color: var(--secondary)">
          <i class="fas fa-edit"></i>
        </button>
        <button class="delete-btn" aria-label="Delete Row" title="Delete Row">
